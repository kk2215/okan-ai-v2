...
const getPlaceId = async (placeName) => {
  try {
    const response = await mapsClient.geocode({ params: { address: placeName, language: 'ja', key: process.env.Maps_API_KEY } });
...
const getRouteInfo = async (departure, arrival) => {
  if (!process.env.Maps_API_KEY) { return 'ごめん、経路検索の準備がまだできてへんみたい…（APIキー未設定）'; }
  try {
