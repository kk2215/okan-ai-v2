...
const createUser = async (userId) => {
  const newUser = {
    ...
    notificationTime: null, trainLines: [], // ★ 複数路線を配列で保存
...
const getTrainStatus = async (trainLineName) => {
  const lineUrlMap = { '山手線': 'https://transit.yahoo.co.jp/diainfo/line/21/0', '埼京線': 'https://transit.yahoo.co.jp/diainfo/line/31/0', '西武池袋線': 'https://transit.yahoo.co.jp/diainfo/line/158/0', '東京メトロ副都心線': 'https://transit.yahoo.co.jp/diainfo/line/456/0' };
  ...
  try {
    ...
    return status ? `・${trainLineName}：『${status}』` : `・${trainLineName}：運行情報が取得できんかったわ`;
  } catch (error) { console.error("Train Info Scraping Error:", error); return `・${trainLineName}：運行情報が取得できんかったわ`; }
};
const getRecipe = () => { /* ... */ };
    ...
    switch (user.setupState) {
      ...
      case 'awaiting_route': {
        ...
        return client.replyMessage(event.replyToken, {
          ...
          quickReply: { items: [...allLines.map(l => ({ type: 'action', action: { type: 'message', label: l, text: l }})), { type: 'action', action: { type: 'message', label: '完了', text: '完了' }}].slice(0,13) }
        });
      }
      case 'awaiting_train_selection': {
        ...
        if (candidates.includes(userText)) {
          ...
          if (!selected.includes(userText)) {
            selected.push(userText);
          }
          ...
          return client.replyMessage(event.replyToken, {
            ...
            quickReply: { items: [...candidates.filter(l => !selected.includes(l)).map(l => ({ type: 'action', action: { type: 'message', label: l, text: l }})), { type: 'action', action: { type: 'message', label: '完了', text: '完了' }}].slice(0,13) }
          });
        } else {
