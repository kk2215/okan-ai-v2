...
const { createLineSelectionMessage } = require('../templates/lineSelectionMessage'); // ★★★ 設計図を呼んでくる ★★★
...
async function handlePostback(event, client) {
    ...
    try {
        ...
        if (action === 'add_line') {
            ...
            const availableLines = user.tempData.availableLines || [];
            
            // 既に選ばれてたら取り消し、なかったら追加する（トグル機能）
            ...
            // ★★★ これがほんまの最後の修正や！ ★★★
            // ただ返事するんやのうて、ボタンの状態を更新した、新しいメッセージを送り直す！
            const updatedMessage = createLineSelectionMessage(availableLines, selectedLines);
            return client.replyMessage(event.replyToken, updatedMessage);
        }
        if (action === 'add_transfer_station') {
            // ★★★ ちゃんと、今まで選んだやつを引き継ぐ！ ★★★
            await updateUserState(userId, 'AWAITING_TRANSFER_STATION', user.tempData);
